...
#define BT_DEVICE_NAME_AD_DATA_LEN                                                                 \
	(BT_AD_DATA_FORMAT_LEN_SIZE + BT_AD_DATA_FORMAT_TYPE_SIZE + BT_DEVICE_NAME_LEN)

/* Maximum manufacturer data length, considering ad data format overhead and
 * the included device name in ad data format.
 */
#define BT_MFG_DATA_LEN_MAX                                                                        \
	(MIN((BT_AD_DATA_FORMAT_LEN_MAX - BT_AD_DATA_FORMAT_TYPE_SIZE),                            \
	     (BT_AD_DATA_LEN_MAX - BT_AD_DATA_FORMAT_LEN_SIZE - BT_AD_DATA_FORMAT_TYPE_SIZE)))
#define BT_MFG_DATA_LEN                                                                            \
	(MIN(BT_MFG_DATA_LEN_MAX, (BT_AD_DATA_LEN_MAX - BT_AD_DATA_FORMAT_LEN_SIZE -               \
				   BT_AD_DATA_FORMAT_TYPE_SIZE - BT_DEVICE_NAME_AD_DATA_LEN)))
/*
 ...
 */
static uint8_t mfg_data[180] = {0xFF, 0xFF};
...
int broadcaster_multiple(void)
{
	...
	for (int index = 0; index < CONFIG_BT_EXT_ADV_MAX_ADV_SET; index++) {
		...
		err = bt_le_ext_adv_set_data(adv[index], ad, ARRAY_SIZE(ad), NULL, 0);
		if (err) {
