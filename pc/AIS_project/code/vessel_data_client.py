...
def get_vessel_data(num_vessels: int = 5) -> list[dict]:
    ...
    try:
        ship_data_respone = requests.get(Config.SHIP_DATA_URL, params={
                                         "num_ships": str(num_vessels)})
    ...
    if ship_data_respone.ok:
        ...
        print(
            f"Server NOT operating properly. SERVER STATUS: {ship_data_respone.status_code}")
        return []
